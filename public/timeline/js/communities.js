...
export async function loadCommunityDetail(communityId) {
  ...
  document
    .querySelector('.community-detail-tab[data-tab="about"]')
...
async function handleEditCommunity(e) {
  ...
  if (iconFile) {
    const webpFile = await convertToWebPAvatar(iconFile, 250, 0.8);
  ...
  if (bannerFile) {
    const webpFile = await convertToWebPBanner(bannerFile, 1500, 500, 0.8);
    const formData = new FormData();
