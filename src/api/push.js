...
const vapidSubject = process.env.VAPID_SUBJECT || `mailto:admin@${process.env.BASE_URL?.replace(/https?:\/\//, "") || "localhost"}`;

if (vapidPublicKey && vapidPrivateKey) {
	webpush.setVapidDetails(vapidSubject, vapidPublicKey, vapidPrivateKey);
}

const getUserByUsername = db.prepare("SELECT id FROM users WHERE LOWER(username) = LOWER(?)");
const getSubscriptionsByUser = db.prepare("SELECT * FROM push_subscriptions WHERE user_id = ?");
const insertSubscription = db.prepare(`
	INSERT INTO push_subscriptions (id, user_id, endpoint, p256dh, auth)
	VALUES (?, ?, ?, ?, ?)
	ON CONFLICT(user_id, endpoint) DO UPDATE SET p256dh = excluded.p256dh, auth = excluded.auth
`);
const deleteSubscription = db.prepare("DELETE FROM push_subscriptions WHERE user_id = ? AND endpoint = ?");
...
export default new Elysia({ prefix: "/push", tags: ["Push Notifications"] })
	...
	.post("/subscribe", async ({ headers, body, set }) => {
	...
	.post("/unsubscribe", async ({ headers, body, set }) => {
		const token = headers.authorization?.replace("Bearer ", "");
		if (!token) {
			set.status = 401;
			return { error: "Unauthorized" };
